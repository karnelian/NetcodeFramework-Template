...
> 아키텍처/패턴은 `SKILL.md` 및 `references/code-patterns.md` 참조.
...
연동: AchievementSystem(달성 이벤트), NotificationSystem(알림), AnalyticsSystem(텔레메트리)
...
UI Prefab 네이밍/계층 규칙은 `ui-workflow` 스킬 참조.
